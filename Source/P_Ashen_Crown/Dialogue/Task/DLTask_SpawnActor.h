#pragma once

#include "P_Ashen_Crown/GameCore.h"
#include "P_Ashen_Crown/TableRow/DialogueTableRow.h"
#include "DLTask_SpawnActor.generated.h"

UCLASS()
class P_ASHEN_CROWN_API UDLTaskParam_SpawnActor : public UDialogueResultParam
{
	GENERATED_BODY()

public:
	UPROPERTY(EditDefaultsOnly, Category = "FindGameplayAttribute")
	TSubclassOf<AActor> mTargetActorClass;

	UPROPERTY(EditDefaultsOnly, Category = "FindGameplayAttribute")
	FTransform mSpawnTransform;
};

/**
 * 액터를 생성하는 Dialogue Task
 */
UCLASS()
class P_ASHEN_CROWN_API UDLTask_SpawnActor : public UDialogueResultTask
{
	GENERATED_BODY()
	
public:
	virtual EDialogueResult ExecuteTask_Implementation(UDialogueResultParam* Param, AActor* Questioner, AActor* Respondent) override;
};
